...
for (const file of ['browser-shell.html', 'browser-shell.js', 'browser-shell.css', 'diagnostics.html']) {
  fs.copyFileSync(
...
// the self-hosted Inter/Space Mono font files under src/renderer/assets/fonts/.
// Copy just the weights browser-shell.css actually declares (Inter
// 400/500/600/700 latin+cyrillic+cyrillic-ext, Space Mono 400 latin-only
// for the address bar) into a fonts/ folder next to the compiled HTML, so
// its @font-face rules can reference them with a plain relative path.
...
for (const file of fs.readdirSync(rendererFontsDir)) {
  if (file.startsWith('Inter-') || file.startsWith('SpaceMono-400-')) {
