...
const uk: Partial<Record<TranslationKey, string>> = {
  ...
  'errors.bulkImportNoNameColumn': 'У цьому файлі немає колонки "name" — це єдина обов\'язкова колонка для масового імпорту.',
  'errors.bulkImportEmptyFile': 'У цьому файлі немає рядків даних для імпорту (лише заголовок, або файл порожній).',
  'errors.bulkImportTooManyRows': 'У цьому файлі забагато рядків для одного масового імпорту.',
  'errors.bulkImportUnreadableFile': 'Не вдалося прочитати цей файл як таблицю — перевірте, що це реальний .csv або .xlsx файл.',
  ...
  'profiles.template.assign': 'Темплейт',
  'profiles.template.adPresetsGroup': 'Пресети рекламних платформ',
  'profiles.template.osLocaleGroup': 'ОС / Локаль',
  ...
  'bulkImport.toolbar.label': 'Масовий імпорт (CSV/XLSX)',
  'bulkImport.toolbar.hint': 'Створити багато профілів одразу з таблиці — завантажити, перевірити, потім підтвердити',
  'bulkImport.title': 'Масовий імпорт профілів',
  'bulkImport.upload.hint': 'Оберіть файл .csv або .xlsx. Обов\'язкова лише колонка "name".',
  'bulkImport.upload.columns':
    'Розпізнавані колонки: name (обов\'язкова), os (windows/macos/linux/android/ios), proxy (має збігатися з назвою наявного проксі), group (створюється автоматично, якщо не існує), tags (через крапку з комою, напр. "affiliate;test").',
  'bulkImport.upload.choose': 'Обрати файл…',
  'bulkImport.preview.summary': '{fileName}: валідних рядків — {valid}, з помилками — {invalid}',
  'bulkImport.col.row': 'Рядок',
  'bulkImport.col.name': 'Назва',
  'bulkImport.col.os': 'ОС',
  'bulkImport.col.proxy': 'Проксі',
  'bulkImport.col.group': 'Група',
  'bulkImport.col.tags': 'Теги',
  'bulkImport.col.status': 'Статус',
  'bulkImport.status.ok': 'OK',
  'bulkImport.chooseAnother': 'Обрати інший файл',
  'bulkImport.confirmCreate': 'Створити {count} профіл(ів)',
  'bulkImport.result.created': 'Створено {count} профіл(ів)',
  'bulkImport.result.skipped': 'Пропущено {count} рядок(ів):',
  ...
  'editor.fingerprint.spoofing.mediaTooltip':
    'Реальні: enumerateDevices() показує справжній список камер/мікрофонів цієї машини, включно зі справжніми лейблами (напр. точні моделі монітора/гарнітури) — однаковими для кожного профілю на цій машині, що є сигналом кореляції, навіть якщо deviceId/groupId і далі підсолені окремо для кожного профілю. Приховані: показує фейковий, консистентний для профілю список пристроїв (той самий seed завжди дає ті самі фейкові пристрої).',
  ...
  'editor.storage.cookies.removed': 'Cookie "{name}" видалено.',
  ...
  'editor.storage.localStorage.removed': 'Запис локального сховища "{key}" видалено.',
  ...
  'editor.advanced.automation.portPlaceholder': 'авто',
  ...
  'editor.advanced.extensions.title': 'Розширення Chrome',
  'editor.advanced.extensions.hint':
    'Завантажує одне чи кілька розпакованих розширень у браузер цього профілю при кожному запуску. Дивіться SECURITY.md — що саме це надає.',
  'editor.advanced.extensions.warning':
    'Розширення отримує всі дозволи, вказані в його маніфесті, у межах цього профілю — воно може читати/змінювати будь-яку сторінку, яку ви відвідуєте, бачити cookies і мережеві запити, зберігати стан — так само, як і реальне встановлене розширення Chrome. Жодного додаткового пісочниці немає, і немає перевірки підпису чи магазину для вмісту розпакованої папки. Додавайте лише розширення, які ви написали самі або яким повністю довіряєте.',
  'editor.advanced.extensions.add': 'Додати розширення…',
  'editor.advanced.extensions.remove': 'Видалити це розширення',
  'editor.advanced.schedule.title': 'Автозапуск за розкладом',
  'editor.advanced.schedule.hint':
    'Автоматично запускати цей профіль у визначений час, у обрані дні, або одноразово в конкретний момент у майбутньому. Вимкнено за замовчуванням. Пропускається, якщо профіль уже запущено.',
  'editor.advanced.schedule.enable': 'Увімкнути автозапуск за розкладом',
  'editor.advanced.schedule.mode': 'Тип розкладу',
  'editor.advanced.schedule.mode.recurring': 'Повторюваний',
  'editor.advanced.schedule.mode.once': 'Одноразовий',
  'editor.advanced.schedule.timezone': 'Часовий пояс',
  'editor.advanced.schedule.timezone.system': 'Системний (локальний час)',
  'editor.advanced.schedule.timezone.hint': 'Застосовується до часу нижче. Залиште "Системний", щоб використовувати локальний час цього комп\'ютера — так само, як і до появи часових поясів для профілю.',
  'editor.advanced.schedule.oneTimeAt': 'Запустити о',
  'editor.advanced.schedule.oneTimeMissing': 'Оберіть дату й час, щоб розклад справді спрацював.',
  'editor.advanced.schedule.oneTimePassed': 'Цей момент уже минув — розклад не спрацює, доки ви не оберете майбутню дату/час.',
  'editor.advanced.schedule.time': 'Час',
  'editor.advanced.schedule.days': 'Дні',
  'editor.advanced.schedule.day.0': 'Нд',
  'editor.advanced.schedule.day.1': 'Пн',
  'editor.advanced.schedule.day.2': 'Вт',
  'editor.advanced.schedule.day.3': 'Ср',
  'editor.advanced.schedule.day.4': 'Чт',
  ...
  'proxy.status.ok': 'OK',
  'proxy.status.failedShort': 'ПОМИЛКА',
  'proxy.status.checking': 'Перевірка…',
  'proxy.status.failed': 'Помилка: {error}',
  'proxy.status.autoOk': 'OK · {when}',
  'proxy.status.autoFail': 'Помилка · {when}',
  'proxy.status.neverChecked': 'Ще не перевірявся',
  'proxy.status.latencySlowHint': 'Вище комфортного порогу 120мс',
  ...
  'proxy.bulkImport.toggle': 'Масовий імпорт',
  'proxy.bulkImport.title': 'Масовий імпорт проксі',
  'proxy.bulkImport.hint': 'По одному проксі на рядок: host:port або host:port:username:password',
  'proxy.bulkImport.placeholder': '203.0.113.5:8080\n203.0.113.6:8080:user:pass',
  'proxy.bulkImport.protocolLabel': 'Протокол (застосується до всіх імпортованих)',
  'proxy.bulkImport.parsedSummary': 'Коректних рядків: {valid}, некоректних: {invalid}',
  'proxy.bulkImport.import': 'Імпортувати',
  'proxy.bulkImport.resultSummary': 'Імпортовано {succeeded} з {total} проксі',
  'proxy.bulkImport.invalidLine': 'Некоректний формат рядка (очікується host:port[:username:password])',
